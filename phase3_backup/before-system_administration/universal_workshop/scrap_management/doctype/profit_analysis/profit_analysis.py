...
from frappe.utils import flt, cint, getdate, today, add_months, date_diff
import json
...
class ProfitAnalysis(Document):
    ...
    def calculate_revenue_metrics(self):
        ...
        try:
            ...
            if self.total_orders > 0:
                ...
                self.highest_order_value_omr = max(
                    flt(invoice.grand_total, 3) for invoice in sales_invoices
                )
            ...
            for invoice in sales_invoices:
                ...
                if invoice.custom_sales_channel:
                    channel_doc = frappe.get_cached_doc(
                        "Sales Channel", invoice.custom_sales_channel
                    )
            ...
            frappe.log_error(f"Error calculating revenue metrics: {str(e)}")
            frappe.throw(
                _("Error calculating revenue metrics. Please check the data and try again.")
            )
    ...
    def calculate_revenue_growth(self):
        ...
        try:
            ...
            prev_invoices = frappe.get_list(
                "Sales Invoice", filters=prev_conditions, fields=["grand_total"]
            )
            ...
            frappe.log_error(f"Error calculating revenue growth: {str(e)}")
    ...
    def calculate_cost_metrics(self):
        ...
        try:
            ...
            frappe.log_error(f"Error calculating cost metrics: {str(e)}")
    ...
    def calculate_labor_costs(self):
        ...
        try:
            ...
            frappe.log_error(f"Error calculating labor costs: {str(e)}")
    ...
    def calculate_storage_costs(self):
        ...
        try:
            ...
            storage_locations = frappe.get_list(
                "Storage Location", fields=["COUNT(*) as location_count"]
            )
            ...
            self.storage_costs_omr = flt(
                location_count * monthly_cost_per_location * period_months, 3
            )

        except Exception as e:
            frappe.log_error(f"Error calculating storage costs: {str(e)}")
    ...
    def calculate_overhead_costs(self):
        ...
        try:
            ...
            frappe.log_error(f"Error calculating overhead costs: {str(e)}")
    ...
    def calculate_shipping_costs(self):
        ...
        try:
            ...
            frappe.log_error(f"Error calculating shipping costs: {str(e)}")
    ...
    def calculate_marketing_costs(self):
        ...
        try:
            ...
            frappe.log_error(f"Error calculating marketing costs: {str(e)}")
    ...
    def calculate_platform_fees(self):
        ...
        try:
            ...
            frappe.log_error(f"Error calculating platform fees: {str(e)}")
    ...
    def calculate_profitability_metrics(self):
        ...
        try:
            ...
            frappe.log_error(f"Error calculating profitability metrics: {str(e)}")
            frappe.throw(
                _("Error calculating profitability metrics. Please check the data and try again.")
            )
    ...
    def calculate_breakeven_point(self):
        ...
        try:
            ...
            frappe.log_error(f"Error calculating breakeven point: {str(e)}")
            self.breakeven_point_days = 0
    ...
    def calculate_inventory_turnover(self):
        ...
        try:
            ...
            frappe.log_error(f"Error calculating inventory turnover: {str(e)}")
            self.inventory_turnover_ratio = 0.0
    ...
    def calculate_financial_ratios(self):
        ...
        try:
            ...
            if estimated_assets > 0:
                self.return_on_assets_percentage = flt(
                    (self.net_profit_omr / estimated_assets) * 100, 2
                )
            else:
            ...
            if estimated_equity > 0:
                self.return_on_equity_percentage = flt(
                    (self.net_profit_omr / estimated_equity) * 100, 2
                )
            else:
            ...
            # Operating margin
            operating_income = (
                self.total_revenue_omr - self.total_costs_omr + (self.overhead_costs_omr or 0)
            )
            if self.total_revenue_omr > 0:
                self.operating_margin_percentage = flt(
                    (operating_income / self.total_revenue_omr) * 100, 2
                )
            else:
                self.operating_margin_percentage = 0.0

        except Exception as e:
            frappe.log_error(f"Error calculating financial ratios: {str(e)}")
    ...
    def generate_performance_data(self):
        ...
        try:
            ...
            frappe.log_error(f"Error generating performance data: {str(e)}")
    ...
    def analyze_part_performance(self):
        ...
        try:
            ...
            item_sales_query = """
                ...
                LIMIT 10
            """.format(
                channel_condition="AND si.custom_sales_channel = %s" if self.sales_channel else ""
            )
            ...
            frappe.log_error(f"Error analyzing part performance: {str(e)}")
    ...
    def analyze_customer_segments(self):
        ...
        try:
            ...
            frappe.log_error(f"Error analyzing customer segments: {str(e)}")
    ...
    def analyze_market_trends(self):
        ...
        try:
            ...
            if self.revenue_growth_percentage > 10:
                ...
                trends.append(
                    "Revenue decline may indicate market saturation or increased competition"
                )
            ...
            if self.marketplace_sales_omr > self.direct_sales_omr:
                recommendations.append(
                    "Marketplace channels are performing well - consider expansion"
                )

            self.recommendations = json.dumps(recommendations, indent=2)

        except Exception as e:
            frappe.log_error(f"Error analyzing market trends: {str(e)}")
            self.market_trends = "[]"
    ...
    def schedule_next_analysis(self):
        ...
        try:
            ...
            frappe.log_error(f"Error scheduling next analysis: {str(e)}")
...
def create_profit_analysis(
    ...
    try:
        ...
        frappe.log_error(f"Error creating profit analysis: {str(e)}")
        return {
            "success": False,
...
def generate_profit_report(filters=None):
    ...
    try:
        ...
        avg_roi = 0
        if analyses:
            avg_margin = sum(flt(a.net_profit_margin_percentage, 2) for a in analyses) / len(
                analyses
            )
        ...
        frappe.log_error(f"Error generating profit report: {str(e)}")
