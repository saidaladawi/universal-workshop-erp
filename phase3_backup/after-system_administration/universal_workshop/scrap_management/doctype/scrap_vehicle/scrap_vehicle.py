...
class ScrapVehicle(Document):
    ...
    def validate_vin_number(self):
        ...
        if self.vin_number:
            ...
            if not vin_pattern.match(self.vin_number):
                frappe.throw(
                    _(
                        "Invalid VIN number format. Must be 17 characters, alphanumeric excluding I, O, Q"
                    )
                )
            ...
            if existing:
                frappe.throw(
                    _("VIN number {0} already exists in Scrap Vehicle {1}").format(
                        self.vin_number, existing
                    )
                )
    ...
    def validate_oman_compliance(self):
        ...
        if self.status in ["In Dismantling", "Dismantled"] and not self.fluids_drained:
            frappe.msgprint(
                _(
                    "Warning: Fluids should be drained before dismantling for environmental compliance"
                )
            )
        ...
        for part in self.extracted_parts:
            if part.item_code and part.storage_location:
                try:
                    ...
                    frappe.log_error(
                        f"Failed to create inventory entry for part {part.part_name}: {e}"
                    )
