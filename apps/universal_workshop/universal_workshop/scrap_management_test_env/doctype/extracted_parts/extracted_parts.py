	...
	def create_inventory_movement(self):
		...
		if self.warehouse:
			try:
				...
				frappe.log_error(f"Failed to create stock entry for {self.name}: {e!s}")
	...
	def update_disassembly_plan_status(self):
		...
		if self.disassembly_plan and self.disassembly_step:
			try:
				...
				frappe.log_error(f"Failed to update disassembly plan {self.disassembly_plan}: {e!s}")
...
def create_barcode_labels(extracted_parts_list):
	...
	for part_name in extracted_parts_list:
		try:
			...
			frappe.log_error(f"Failed to generate label for {part_name}: {e!s}")
...
def update_part_availability(part_name, new_status, notes=None):
	...
	try:
		...
		frappe.log_error(f"Failed to update status for {part_name}: {e!s}")
