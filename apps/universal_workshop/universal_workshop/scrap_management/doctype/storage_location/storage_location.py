...
class StorageLocation(Document):
	...
	def calculate_efficiency_metrics(self):
		...
		utilizations = [
			u for u in [self.weight_utilization, self.volume_utilization, self.item_utilization] if u > 0
		]
		if utilizations:
	...
	def get_available_capacity(self):
		...
		return {
			"weight_available_kg": (
				flt(self.max_weight_kg - (self.current_weight_kg or 0), 2) if self.max_weight_kg else None
			),
			"volume_available_m3": (
				flt(self.max_volume_m3 - (self.current_volume_m3 or 0), 3) if self.max_volume_m3 else None
			),
			"items_available": (
...
def check_location_capacity(location, part_weight, part_volume):
	...
	if location.get("max_weight_kg"):
		available_weight = flt(location["max_weight_kg"]) - flt(location.get("current_weight_kg", 0))
		if part_weight > available_weight:
			return False, _("Exceeds weight capacity")

	# Volume check
	if location.get("max_volume_m3"):
		available_volume = flt(location["max_volume_m3"]) - flt(location.get("current_volume_m3", 0))
