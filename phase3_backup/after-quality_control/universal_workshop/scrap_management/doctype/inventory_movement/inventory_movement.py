...
from frappe.utils import flt, cint, now_datetime, get_datetime, time_diff_in_seconds
import json
import re
...
class InventoryMovement(Document):
    ...
    def validate_movement_logic(self):
        ...
        if self.movement_type in ["Stock In", "Found"]:
            if not self.to_location:
                frappe.throw(
                    _("To Location is required for {0} movement").format(self.movement_type)
                )
        ...
        # Stock Out and Loss movements require only 'from_location'
            ...
            if not self.from_location:
                frappe.throw(
                    _("From Location is required for {0} movement").format(self.movement_type)
                )
            if self.to_location:
    ...
    def validate_location_capacity(self):
        ...
        if self.to_location and self.movement_type in ["Stock In", "Transfer", "Found"]:
            ...
            if not can_accommodate:
                frappe.throw(
                    _("Destination location cannot accommodate this part: {0}").format(reason)
                )
        ...
        if self.scanned_barcode:
            ...
            # Validate location barcode format (LOC-LOCATIONCODE format)
                ...
                if not frappe.db.exists("Storage Location", {"location_code": location_code}):
                    frappe.throw(
                        _("Location with barcode {0} not found").format(self.scanned_barcode)
                    )
    ...
    def validate_quantities(self):
        ...
        if self.expected_quantity and self.actual_quantity:
            if (
                abs(self.expected_quantity - self.actual_quantity) > 0.01
            ):  # Allow small precision differences
                if not self.discrepancy_reason:
                    frappe.throw(
                        _(
                            "Discrepancy reason is required when expected and actual quantities differ"
                        )
                    )
    ...
    def validate_approval_requirements(self):
        ...
        if self.extracted_part:
            part_value = frappe.get_value(
                "Extracted Parts", self.extracted_part, "estimated_price_omr"
            )
...
def get_movement_analytics(warehouse=None, date_range=None):
    ...
    for movement in movements:
        ...
        analytics["by_verification"][ver_status] = (
            analytics["by_verification"].get(ver_status, 0) + 1
        )
...
def bulk_movement_import(movement_data):
    ...
    for movement_item in movement_data:
        try:
            ...
            results.append(
                {"success": True, "movement_id": movement.name, "part_code": movement.part_code}
            )
