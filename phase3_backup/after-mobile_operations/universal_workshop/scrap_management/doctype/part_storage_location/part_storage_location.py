...
class PartStorageLocation(Document):
    ...
    def validate_part_code(self):
        ...
        if not re.match(r'^PSL-\d{4}-\d{5}$', self.part_code):
            frappe.throw(_(
                "Part code must be in format 'PSL-YYYY-NNNNN'"
            ))
    ...
    def validate_barcode_uniqueness(self):
        ...
        if self.barcode:
            existing = frappe.db.exists(
                ...
                {"barcode": self.barcode, "name": ("!=", self.name or "")}
            )
            if existing:
    ...
    def validate_storage_zone_compatibility(self):
        ...
        if self.storage_zone and self.part_category:
            ...
            can_accommodate, message = zone.can_accommodate_part(
                self.part_category, self.weight_kg or 0
            )
    ...
    def log_movement(self, movement_type, from_location, to_location, 
    ...
    def move_to_location(self, new_zone, new_rack=None, new_shelf=None, 
                        new_bin=None, reason=None, notes=None):
    ...
    def calculate_current_value(self):
        ...
        depreciated_value = self.estimated_value * (
            (100 - self.depreciation_rate) / 100
        ) ** months_stored
...
@frappe.whitelist()
def find_optimal_storage_location(part_category, weight_kg=0, 
    ...
    zones = frappe.get_list(
        "Storage Zone",
        filters=filters,
        fields=["name", "zone_code", "zone_name", "utilization_percentage"]
    )
        ...
        if can_accommodate:
            ...
            suitable_zones.append({
                **zone_data,
                ...
                "available_capacity": zone.get_available_capacity()
            })
...
def get_parts_due_for_inspection():
    ...
    parts = frappe.get_list(
        ...
        fields=[
            "name", "part_code", "item_name", "storage_zone",
            "current_location", "last_inspection_date", "next_inspection_date"
        ],
...
def get_storage_analytics():
    ...
    parts = frappe.get_all(
        ...
        fields=[
            "part_category", "estimated_value", "storage_cost_per_month",
            "market_demand", "priority_to_sell", "date_stored", "weight_kg"
        ]
    )
