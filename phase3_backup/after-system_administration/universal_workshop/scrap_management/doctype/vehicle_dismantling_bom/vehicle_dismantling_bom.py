...
class VehicleDismantlingBOM(Document):
    ...
    def validate_vehicle_data(self):
        ...
        if cint(self.year_from) < 1900 or cint(self.year_to) > current_year + 2:
            frappe.throw(
                _("Invalid year range. Must be between 1900 and {0}").format(current_year + 2)
            )
    ...
    def validate_operations(self):
        ...
        for operation in self.dismantling_operations:
            ...
            if operation.operation_sequence in sequences:
                frappe.throw(
                    _("Duplicate operation sequence: {0}").format(operation.operation_sequence)
                )
            ...
            if operation.estimated_time <= 0:
                frappe.throw(
                    _("Operation '{0}' must have positive estimated time").format(
    ...
    def validate_extractable_parts(self):
        ...
        for part in self.extractable_parts:
            ...
            if part.estimated_value < 0:
                frappe.throw(
                    _("Part '{0}' estimated value cannot be negative").format(part.part_name)
                )

            if (
                part.minimum_selling_price > part.maximum_selling_price
                and part.maximum_selling_price > 0
            ):
                frappe.throw(
                    _("Part '{0}' minimum price cannot exceed maximum price").format(part.part_name)
                )

            # Validate extraction time
            if part.estimated_extraction_time <= 0:
                frappe.throw(
                    _("Part '{0}' must have positive extraction time").format(part.part_name)
                )
            ...
            if part.part_name_ar and not self.is_arabic_text(part.part_name_ar):
                frappe.throw(
                    _("Part '{0}' Arabic name must contain Arabic characters").format(
                        part.part_name
                    )
                )
    ...
    def validate_bom_conflicts(self):
        ...
        for bom in existing_boms:
            ...
            if cint(self.year_from) <= cint(bom.year_to) and cint(self.year_to) >= cint(
                bom.year_from
            ):
                frappe.throw(
    ...
    def create_work_order_template(self):
        ...
        try:
            ...
            frappe.log_error(f"Error creating work order template: {str(e)}")
    ...
    def get_roi_projection(self, acquisition_cost=0):
        ...
        # Total costs
        total_costs = (
            acquisition_cost + total_extraction_cost + total_refurbishment + total_disposal_cost
        )
