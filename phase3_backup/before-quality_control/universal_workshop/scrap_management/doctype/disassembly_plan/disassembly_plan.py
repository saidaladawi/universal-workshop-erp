...
class DisassemblyPlan(Document):
    ...
    def validate_scrap_vehicle_status(self):
        ...
        if vehicle.status not in ["Assessment Complete", "Dismantling Planned"]:
            frappe.throw(
                _(
                    "Scrap Vehicle must have completed condition assessment before creating dismantling plan"
                )
            )
    ...
    def optimize_custom(self, parts):
        ...
        for part in parts:
            ...
            part["combined_score"] = (
                value_score * 0.4 + safety_score * 0.3 + accessibility_score * 0.3
            )
    ...
    def complete_step(self, step_number, actual_time_minutes=None, technician_notes=None):
        ...
        if next_step:
            frappe.msgprint(
                _("Step {0} completed. Step {1} started.").format(
