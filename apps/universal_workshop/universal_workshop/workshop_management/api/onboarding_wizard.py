...
def complete_onboarding(progress_id):
    ...
    try:
        ...
        # Get license type for module configuration
        license_type = "basic"  # Default
        if license_has_data:
            business_reg = frappe.get_list(
            ...
            if business_reg:
                license_type = business_reg[0].get("license_type", "basic")
        
        # Configure modules based on license
        module_config_result = configure_modules_for_license(license_type)
        ...
        return {
            ...
            "modules_configured": module_config_result.get("enabled_modules", []),
...
def check_onboarding_status():
    """Check if onboarding has been completed for the system"""
    try:
        ...
        workshop_exists = frappe.db.exists("Workshop Profile", {"docstatus": 0})
        
        # Check if system settings are initialized
        settings = frappe.get_doc("Universal Workshop Settings")
        settings_complete = getattr(settings, 'setup_completed', False)
        
        # Check if there are any completed onboarding sessions
        completed_onboarding = frappe.db.exists("Onboarding Progress", {"docstatus": 1})
        
        # Determine if onboarding should be shown
        show_onboarding = not (setup_complete and workshop_exists and settings_complete)
        
        # Get license mode if available
        license_has_data = frappe.db.get_default("license_has_workshop_data") == "1"
        ...
        result = {
            ...
            "completed_onboarding": bool(completed_onboarding),
            "license_mode": license_has_data,
            "can_skip_onboarding": False  # Always require onboarding for new systems
        }
        
        # If onboarding is complete, get workshop info
        if not show_onboarding and workshop_exists:
            workshop = frappe.get_doc("Workshop Profile", workshop_exists)
            result.update({
        ...
        frappe.log_error(f"Error checking onboarding status: {str(e)}")
...
@frappe.whitelist()
def get_onboarding_step_fields(step_name):
    """Get field definitions for onboarding step"""
    fields_config = {
        "license_verification": {
            "title": _("License Verification & Welcome"),
            "description": _("Enter your business license key to verify and extract workshop information"),
            "fields": [
                {
                    "name": "license_key",
                    "label": _("Business License Key"),
                    "type": "data",
                    "required": True,
                    "placeholder": _("Enter 7-digit Oman business license number")
                },
                {
                    ...
                    "options": [
                        ...
                        {"value": "both", "label": _("Both Arabic & English")}
                    ],
                    "required": True,
                    "default": "both"
                },
        ...
        "admin_account": {
            "title": _("Administrator Account Creation"),
            "description": _("Create the main administrator account for your workshop"),
            "fields": [
                {
                    "name": "admin_username",
                    "label": _("Username"),
                    "type": "data", 
                    "required": True,
                    "placeholder": _("Enter admin username")
                },
                {
                    "name": "admin_password",
                    "label": _("Password"),
                    "type": "password",
                    "required": True,
                    "placeholder": _("Enter secure password")
                },
                {
                    "name": "admin_email",
                    ...
                    "type": "email",
                    "required": True,
                    "placeholder": _("admin@yourworkshop.com")
                },
                {
                    "name": "admin_phone",
                    ...
                    "type": "data",
                    "required": True,
                    "placeholder": _("+968 XXXXXXXX")
                }
            ]
        },
        "system_configuration": {
            "title": _("Essential System Configuration"),
            "description": _("Configure basic operational settings for your workshop"),
            "fields": [
                {
                ...
                },
                {
                    ...
                    "type": "select",
                    "options": [
                        {"value": "OMR", "label": _("Omani Rial (OMR)")},
                        {"value": "USD", "label": _("US Dollar (USD)")},
                        {"value": "EUR", "label": _("Euro (EUR)")}
                    ],
                    "required": True,
                    "default": "OMR"
                },
                {
                    "name": "vat_registered",
                    "label": _("VAT Registered"),
                    "type": "check",
                    "required": False,
                    "description": _("Check if your workshop is registered for VAT")
                }
            ]
        }
    }
    
    return fields_config.get(step_name, {})


@frappe.whitelist()
def enable_fresh_startup():
    """Enable fresh startup mode to re-run onboarding wizard"""
    try:
        # Check if user has permission
        if not frappe.has_permission("Universal Workshop Settings", "write"):
            frappe.throw(_("You don't have permission to reset onboarding"))
        
        # Reset onboarding flags
        frappe.db.set_default("setup_complete", "0")
        frappe.db.set_default("license_has_workshop_data", "0")
        
        # Update system settings
        settings = frappe.get_doc("Universal Workshop Settings")
        settings.setup_completed = 0
        settings.onboarding_completed_on = None
        settings.onboarding_completed_by = None
        ...
        return {
            "success": True,
            "message": _("Fresh startup mode enabled. Onboarding wizard will appear on next visit.")
        }
        
    except Exception as e:
        frappe.log_error(f"Error enabling fresh startup: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
