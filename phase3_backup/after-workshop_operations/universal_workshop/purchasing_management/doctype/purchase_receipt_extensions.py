...
class PurchaseReceiptQualityInspectionController:
    ...
    def update_inspection_status(self):
        ...
        failed_items = [item for item in self.doc.items if item.inspection_status == "Failed"]

        if failed_items:
            self.doc.quality_inspection_status = "Failed"
            self.doc.quality_inspection_status_ar = "فشل"
        elif pending_items:
            if any(item.inspection_status == "In Progress" for item in self.doc.items):
    ...
    def validate_inspection_completion(self):
        ...
        if self.doc.docstatus == 1:  # On submission
            if self.doc.quality_inspection_required and self.doc.total_inspections_pending > 0:
                frappe.throw(
                    _(
                        "Cannot submit Purchase Receipt. {0} quality inspections are still pending."
                    ).format(self.doc.total_inspections_pending)
                )
...
def update_purchase_receipt_inspection_status(purchase_receipt):
    ...
    return {
        "status": pr_doc.quality_inspection_status,
        "pending_inspections": pr_doc.total_inspections_pending,
    }
...
def get_inspection_summary(purchase_receipt):
    ...
    summary = {
        ...
        "items_requiring_inspection": len(
            [item for item in pr_doc.items if item.quality_inspection_required]
        ),
        "inspections_completed": sum([item.inspections_completed for item in pr_doc.items]),
        "inspections_pending": pr_doc.total_inspections_pending,
        "overall_status": pr_doc.quality_inspection_status,
        "items_detail": [],
    }
            ...
            summary["items_detail"].append(
                {
                    ...
                    "batch_inspections_required": item.batch_inspections_required,
