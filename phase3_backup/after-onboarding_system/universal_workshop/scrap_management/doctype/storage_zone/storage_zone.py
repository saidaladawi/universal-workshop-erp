...
class StorageZone(Document):
    ...
    def validate_zone_code(self):
        ...
        if self.zone_code:
            if not re.match(r'^[A-Z]{1,3}-\d{2,3}$', self.zone_code):
                frappe.throw(_(
                    "Zone code must be in format 'A-01' or 'ABC-123'"
                ))
    ...
    def validate_arabic_fields(self):
        ...
        if self.zone_name_ar and not self.contains_arabic(self.zone_name_ar):
            frappe.throw(_(
                "Zone name Arabic field must contain Arabic characters"
            ))
    ...
    def validate_capacity(self):
        ...
        if self.max_capacity and self.current_capacity:
            if self.current_capacity > self.max_capacity:
                frappe.throw(_(
                    "Current capacity cannot exceed maximum capacity"
                ))
    ...
    def validate_allowed_categories(self):
        ...
        restricted_count = sum(
            1 for cat in self.allowed_categories if cat.is_restricted
        )
        if self.zone_type == "General" and restricted_count > 0:
            frappe.throw(_(
                "General zones cannot have restricted part categories"
            ))
    ...
    def can_accommodate_part(self, part_category, weight_kg=0):
        ...
        allowed_categories = [
            cat.part_category for cat in self.allowed_categories
        ]
    ...
    def add_part_to_zone(self, part_storage_location):
        ...
        can_add, message = self.can_accommodate_part(
            part_storage_location.part_category,
            part_storage_location.weight_kg or 0
        )
        ...
        # Log the addition
        self.log_activity(
            f"Part {part_storage_location.part_code} added to zone"
        )
    ...
    def remove_part_from_zone(self, part_storage_location):
        ...
        if self.current_capacity and self.current_capacity > 0:
            ...
            # Log the removal
            self.log_activity(
                f"Part {part_storage_location.part_code} removed from zone"
            )
...
def get_available_zones(part_category=None, weight_kg=0):
    ...
    zones = frappe.get_list(
        ...
        filters=filters,
        fields=[
            "name", "zone_code", "zone_name", "zone_type",
            "max_capacity", "current_capacity", "utilization_percentage"
        ]
    )
    ...
    for zone_data in zones:
        ...
        if part_category:
            can_accommodate, _ = zone.can_accommodate_part(
                part_category, weight_kg
            )
            if can_accommodate:
...
def get_zone_utilization_summary():
    ...
    if summary["total_capacity"] > 0:
        summary["overall_utilization"] = (
            summary["total_used"] / summary["total_capacity"]
        ) * 100
    else:
